...
> **How to invoke**: Type `#<prompt-name>` in GitHub Copilot Chat (e.g., `#forge-init`, `#forge-spec`, `#learn`).
...
|--------|----------|-----------|
| [`#forge-init`](#forge-init) | Setup | Bootstrap `.forge/` in a new repo |
| [`#forge-spec`](#forge-spec) | Planning | Write a requirement spec from a feature request |
| [`#forge-architect`](#forge-architect) | Planning | Design architecture and break a requirement into tasks |
| [`#forge-validate`](#forge-validate) | Quality Gate | Validate any phase output before advancing |
| [`#forge-tdd`](#forge-tdd) | Testing | Generate failing test suites before implementation |
| [`#forge-proceed`](#forge-proceed) | Orchestration | Run the full pipeline end-to-end for a single REQ |
| [`#forge-sprint`](#forge-sprint) | Orchestration | Batch-run multiple `#forge-proceed` pipelines sequentially |
| [`#forge-check-drift`](#forge-check-drift) | Review | Lightweight read-only convention compliance check |
| [`#forge-reflect`](#forge-reflect) | Review | Post-implementation self-review before formal review |
| [`#forge-review`](#forge-review) | Review | Multi-dimension code review |
| [`#forge-bugfix`](#forge-bugfix) | Bug Fixing | End-to-end bug fix workflow |
| [`#forge-vibe`](#forge-vibe) | Lightweight | Fast, low-overhead workflow for trivial changes |
| [`#forge-query`](#forge-query) | Knowledge | Query project knowledge and optionally capture new insights |
| [`#forge-wrapup`](#forge-wrapup) | Completion | Close out a feature — merge, deploy, capture knowledge |
| [`#forge-canary`](#forge-canary) | Deployment | Canary deploy with smoke tests |
| [`#forge-deploy`](#forge-deploy) | Deployment | Execute deployment steps from deployment config |
| [`#forge-issue-epic-creation`](#forge-issue-epic-creation) | Integration | Sync REQs and Tasks to GitLab as Epics and Issues |
| [`#forge-status`](#forge-status) | Visibility | Show current state of all Copilot Forge work |
| [`#forge-analyze`](#forge-analyze) | Health | Codebase health audit |
| [`#forge-security-scan`](#forge-security-scan) | Security | Pre-commit secret and credential audit |
...
```
#forge-init → #forge-spec → #forge-validate → #forge-architect → #forge-validate → #forge-tdd → implement → #forge-reflect → #forge-review → merge → #forge-wrapup → #forge-synthesize (periodically)
```

Or automated via `#forge-proceed` (single REQ) or `#forge-sprint` (batch):

```
#forge-proceed REQ-xxx     ← runs all phases automatically
#forge-sprint REQ-001 REQ-002 REQ-003   ← runs #forge-proceed for each REQ sequentially
```

Standalone utilities can be invoked at any time:

```
#forge-query       — query project knowledge and optionally capture new insights
#forge-bugfix      — fix a bug end-to-end
#forge-vibe        — quick change without heavy specs
#forge-check-drift — lightweight read-only convention compliance check
#forge-analyze     — codebase health audit
#forge-optimize    — performance scanner
#forge-status      — see what's in progress
#forge-canary      — canary deploy with smoke tests
#forge-security-scan — scan for secrets before committing
#forge-admin — modify the Forge toolkit itself safely
#forge-template-drift — check template freshness
#forge-token-estimate — estimate session token cost
#forge-synthesize  — process learning inbox into permanent knowledge
#forge-prune       — archive stale rules and lessons
```
...
| **Next step** | `#forge-spec` to write your first requirement. |
...
| **Next step** | `#forge-validate` to verify architecture and tasks. |
...
| **Next step** | Depends on phase: Spec → `#forge-architect`, Architecture → implement, Tasks → implement, Implementation → `#forge-review`. |
...
| | |
|---|---|
| **When to call** | After `#forge-architect` has produced tasks and before implementation begins. Part of Phase 3.5 in `#forge-proceed`. |
| **Input** | `REQ-xxx` ID. |
...
| **Input** | `REQ-xxx` ID (e.g., `#forge-proceed REQ-023`). |
...
| | |
|---|---|
| **When to call** | Intermittently during active development (or automatically inside `#forge-proceed`) to catch convention violations early. |
...
> Post-implementation self-review — catch issues before the formal `#forge-review` step.

| | |
|---|---|
| **When to call** | After implementing a feature, before running `#forge-review`. Think of it as "checking your own work." |
...
| **Next step** | `#forge-review` for formal multi-dimension code review. |
...
| | |
|---|---|
| **When to call** | After implementation and `#forge-reflect`. This is the formal quality gate before merge. |
...
| **Next step** | Fix critical findings, then `#forge-wrapup` or merge. |
...
| **What it does** | 7 phases:<br><br>**Phase 1 — Report**: Creates `BUG-xxx-slug.md` using the bug template, or reads existing report. Cross-repo: adds `repo:` or `touched_repos:` to frontmatter.<br>**Phase 2 — Analyze**: Traces code paths, identifies root cause (not just symptoms), documents in the bug report.<br>**Phase 3 — Fix**: Creates `fix/bug-xxx-slug` branch, implements fix following conventions, updates tests.<br>**Phase 4 — Verify**: Runs test suite, updates bug report with resolution details.<br>**Phase 5 — Ship**: Pushes branch, creates PR via `gh pr create`, waits for CI.<br>**Phase 6 — Canary** (optional): Runs `#forge-canary` for high/critical severity bugs touching deployable services.<br>**Phase 7 — Wrapup**: Merges PR, confirms deploys (staging + production), captures lesson, cleans up branches/worktrees, emits ship summary. |
...
| | |
|---|---|
| **When to call** | For small, isolated changes that don't need the full SDD pipeline. `#forge-spec` will automatically suggest `#forge-vibe` when appropriate. Good for: cosmetic fixes, config tweaks, copy changes, single-file edits. |
...
| **Prerequisites** | `.forge/knowledge/` must exist (run `#forge-init` if not). |
...
| | |
|---|---|
| **When to call** | Periodically — after a few `#forge-reflect`, `#forge-review`, or `#forge-wrapup` sessions have deposited candidates into the inbox. |
| **Input** | None — processes all files in `.forge/knowledge/inbox/`. |
| **Prerequisites** | `.forge/knowledge/inbox/` must exist (scaffolded by `#forge-init`). At least one candidate file must be present. |
...
| | |
|---|---|
| **When to call** | After a feature has been implemented and reviewed. This is the final step before moving on. Usually invoked automatically at the end of `#forge-proceed`. |
| **Input** | `REQ-xxx` ID (or inferred from current branch / recent merges). |
| **Prerequisites** | Feature implementation complete, ideally reviewed via `#forge-review`. |
...
| | |
|---|---|
| **When to call** | Before promoting a new version to production. Used automatically in `#forge-proceed` Phase 7.5 for deployable requirements, and in `#forge-bugfix` Phase 6 for high-severity bugs. Can also be invoked standalone. |
...
| **Prerequisites** | `.forge/context/deployment.md` must exist (created by `#forge-init`). |
...
### `#forge-issue-epic-creation`

> Synchronize local REQs and Tasks to GitLab as Epics and Issues.

| | |
|---|---|
| **When to call** | After `#forge-architect` has produced tasks and before starting implementation, if your team uses GitLab for issue tracking. |
...
| | |
|---|---|
| **When to call** | Before committing sensitive code changes, or anytime you want to verify no secrets are in the diff. Runs automatically as part of `#forge-wrapup` Step 2. |
...
| | |
|---|---|
| **When to call** | Before or after a `#forge-proceed` run to understand token costs. For routine use, prefer running `.\scripts\token-estimate.ps1` directly from the terminal (zero Copilot token cost). |
...
|-----------------|---------|-------|
| `reflector` | `#forge-reflect`, `#forge-proceed` Phase 5 | Self-review: correctness, conventions, completeness |
| `correctness-reviewer` | `#forge-review`, `#forge-proceed` Phase 5 | Logic errors, null risks, race conditions, security |
| `quality-reviewer` | `#forge-review`, `#forge-proceed` Phase 5 | Naming, conventions, duplication, validation |
| `architecture-reviewer` | `#forge-review`, `#forge-proceed` Phase 5 | Layering, separation of concerns, API contracts |
| `test-auditor` | `#forge-review`, `#forge-analyze`, `#forge-proceed` Phase 5 | Coverage gaps, mock completeness, test quality |
| `security-auditor` | `#forge-review`, `#forge-analyze`, `#forge-proceed` Phase 5 | Injection, auth, data exposure, rate limiting |
| `feature-tracer` | `#forge-architect` | Traces similar existing implementations |
| `architecture-mapper` | `#forge-architect` | Maps affected files and dependency layers |
| `integration-explorer` | `#forge-architect` | Identifies extension points and API contracts |
| `convention-auditor` | `#forge-analyze` | Convention compliance checking |
| `code-quality-auditor` | `#forge-analyze` | Dead code, duplication, complexity |
| `api-cost-scanner` | `#forge-optimize` | AI API call inventory and cost estimation |
| `db-perf-scanner` | `#forge-optimize` | Database query performance analysis |
| `latency-scanner` | `#forge-optimize` | Request path latency analysis |
| `task-implementer` | `#forge-proceed` Phase 4 | Task implementation guidelines |
...
Start
  ...
  │    │    └─ #forge-proceed REQ-xxx  (or #forge-sprint for batch)
  ...
  │    ├─ Code quality & tech debt → #forge-analyze
  │    ├─ Performance & costs → #forge-optimize
  │    ├─ Secrets in code → #forge-security-scan
  │    └─ Template freshness → #forge-template-drift
  │
  ...
  │
  ├─ Want visibility?
  │    ├─ What's in progress → #forge-status
  │    └─ Token costs → #forge-token-estimate
  │
  ├─ Need to deploy?
  │    ├─ Standard deploy → #forge-deploy
  │    └─ Canary (zero-traffic first) → #forge-canary
  │
