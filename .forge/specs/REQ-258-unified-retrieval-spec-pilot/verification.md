...
**Date:** 2026-04-19
**Worktree:** `.worktrees/magical-mendeleev-c4d3ab`
...
**Evidence sources:**
- `spec/prompt.md:144` — Step 3.3 explicitly: *"**Retrieved Context** (NEW, always present): append a `## Retrieved Context` section at the end of the spec listing every retrieved source from Step 1.6.8 in the form `ID (corpus, score): title`."*
- `.forge/specs/REQ-258-unified-retrieval-spec-pilot/requirement.md:149-151` — REQ-258 carries its own `## Retrieved Context` section at the end.
...
- `spec/prompt.md:144` — Step 3.3 bullet: *"If no context was retrieved (cold-start path), write exactly: `No prior context retrieved — first REQ in this area.`"* — matches BR-9/AC-5 text exactly, character-for-character (both use the em-dash `—`).
- `.forge/specs/REQ-258-unified-retrieval-spec-pilot/requirement.md:151` — REQ-258 itself is a cold-start and writes: *"No prior context retrieved — first REQ tracked in the copilot-forge repo."* NOTE: REQ-258's wording is expanded beyond the canonical sentence ("first REQ tracked in the copilot-forge repo" vs "first REQ in this area"). This is authorial elaboration on the self-bootstrap context and does not contradict the prompt's instruction — the prompt specifies the canonical wording for generated REQs; REQ-258 was hand-authored. See Finding F-1 below for assessment.

**Status:** **PASS (static — canonical wording is present and exact in the prompt at line 144). Runtime behavior deferred to dogfood Scenario A** (§5.1).
...
**Evidence source:** `spec/prompt.md:82-88` (Step 1.6.3 scoring rule)
...
**Evidence source:** `spec/prompt.md:94` (Step 1.6.6)
...
**Evidence sources:**
- `templates/requirement-template.md:8-12` — exposes `component`, `domain`, `stack`, `concerns`, `tags` each with an explanatory `#` comment (e.g., line 8: `component: ""       # narrow area, e.g., "API/auth", "iOS/SwiftUI", "Copilot Forge/spec"`).
...
- `init/prompt.md:157-177` — new Step 7 "Scaffold Retrieval Taxonomy" with idempotent copy logic (line 171: `if [ ! -f .forge/context/taxonomy.md ]; then cp ... fi`) and a guard that errors if the canonical template is missing (lines 164-167).
...
**What was verified:**
- `spec/prompt.md:31-34` — Step 1 has only three sub-items: "Read project-overview.md", "Read architecture.md", "Ask clarifying questions". There is **no Step 1.3** implementing the old 3-tier lesson grep.
- `spec/prompt.md:36-67` — Step 1.5 (query derivation) is brand new.
- `spec/prompt.md:69-107` — Step 1.6 (unified retriever) is the sole retrieval behavior.
- A grep for the old 3-tier phrases (`component exact`, `domain + prefix`, or any lingering fallback to lesson-only grep) in `spec/prompt.md` turns up only the prose on line 71 that describes what was removed: *"This is the only retrieval behavior — the prior 3-tier lesson grep is removed."* No residual 3-tier logic exists.
...
- Line 12: `stack: []           # tech layers touched, e.g., ["swift", "postgres"]` — **added** per AC-11.
...
To confirm the formula in `spec/prompt.md:82-88` matches REQ-258 BR-1 line-for-line, this section computes scores for a synthetic query against four synthetic documents and compares against the expected ordering stated in the task file's Scenario B (lines 130-134).
...
Matches the task file expectation and confirms the "specs and bugs with zero tag overlap score 0" branch of BR-1 (prompt line 88).
...
Validates the lesson-only foundational floor branch (BR-1 final bullet; prompt line 88).
...
| BR-1 clause (requirement.md line 79-84) | prompt text (spec/prompt.md line 83-88) | Match? |
|---|---|---|
...
**Conclusion:** the prompt's scoring formula is a character-level match for BR-1.
...
| AC-5 | PASS (canonical wording exact in prompt) | Runtime cold-start emission deferred to Scenario A |
...
These scenarios execute the original TASK-006 plan once the REQ-258 PR is merged and `.forge` points at the updated main. They confirm the runtime-observable aspects of AC-1, AC-4, and AC-5 that cannot be seen from committed files alone.

### 5.1 Scenario A — Cold-Start (verifies AC-5 runtime emission; also covers AC-2, AC-3)

**Setup:**
```bash
mkdir -p /tmp/req258-scenario-a/.forge/{knowledge/lessons,specs,bugs,context}
cd /tmp/req258-scenario-a
echo "# Test project — a synthetic workspace for REQ-258 cold-start verification." > .forge/context/project-overview.md
echo "# Test architecture — flat project, no layers." > .forge/context/architecture.md
echo "# Test conventions — none." > .forge/context/conventions.md
# Optional: copy the ETHOS and templates so prompts work consistently
cp .forge .forge/ETHOS.md 2>/dev/null || true
mkdir -p .forge/templates && cp .forge .forge/templates/ 2>/dev/null || true
```
...
1. The prompt runs Step 1.5 and surfaces a proposed query such as:
   ```
   ...
   ```
2. After user confirmation, Step 1.6 enumerates `.forge/knowledge/lessons/`, `.forge/specs/`, and `.forge/bugs/` — all empty — and takes the cold-start path (Step 1.6.9).
3. No retrieval summary is printed (or a summary reading `Retrieved context: none — cold-start path`).
4. Step 3 writes `.forge/specs/REQ-xxx-add-sso.../requirement.md` with:
...
**Rollback if AC-5 fails:** a Phase 5 finding to re-verify `spec/prompt.md:107` and `spec/prompt.md:144` (the cold-start branch and the canonical sentence). Fix in TASK-004's footprint, re-run Scenario A.
...
```bash
mkdir -p /tmp/req258-scenario-b/.forge/{knowledge/lessons,specs,bugs,context}
cd /tmp/req258-scenario-b
for f in project-overview architecture conventions; do echo "# $f" > ".forge/context/$f.md"; done
cp .forge .forge/ETHOS.md 2>/dev/null || true
mkdir -p .forge/templates && cp .forge .forge/templates/ 2>/dev/null || true

# Two lessons
cat > .forge/knowledge/lessons/LESSON-001-auth-tokens.md <<'EOF'
---
id: LESSON-001
...
cat > .forge/knowledge/lessons/LESSON-002-ui-snapshots.md <<'EOF'
---
id: LESSON-002
...
# Two bugs
cat > .forge/bugs/BUG-001-rate-limit.md <<'EOF'
---
id: BUG-001
...
cat > .forge/bugs/BUG-002-unrelated.md <<'EOF'
---
id: BUG-002
...
**Agent-executed behavior** (following updated spec/prompt.md):
...
- Step 3 wrote the generated spec to `/tmp/req258-scenario-a/.forge/specs/REQ-999-add-sso-admin/requirement.md`.
...
```
Section present. Canonical wording matches the prompt's Step 3 instruction character-for-character (updated Phase 5 wording).
...
**Setup** — scratch corpus with controlled tags (see `/tmp/req258-scenario-b/.forge/`):
...
- Step 3 wrote `/tmp/req258-scenario-b/.forge/specs/REQ-998-password-reset-email/requirement.md` with inline citations on load-bearing rules.
...
| AC-9 | PASS (static) | **PASS** | init/prompt.md Step 7 |
| AC-10 | PASS (static) | **PASS** | spec/prompt.md Step 1.6 |
