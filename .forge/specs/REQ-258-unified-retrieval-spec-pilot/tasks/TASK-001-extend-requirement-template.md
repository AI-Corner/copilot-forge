...
- [ ] Each field has an inline comment (using `#` YAML comment syntax) documenting its purpose and citing a representative example, e.g. `component: ""  # narrow area, e.g., "API/auth" or "Copilot Forge/spec"`.
...
**Dogfooding already present**: `.forge/specs/REQ-258-unified-retrieval-spec-pilot/requirement.md` already uses this shape (with populated values) as a reference. Do not confuse dogfooded values for template defaults.
