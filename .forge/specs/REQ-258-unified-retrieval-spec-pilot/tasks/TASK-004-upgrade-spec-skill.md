...
- `spec/prompt.md` — multi-section edit per the detailed spec below.
...
- [ ] Cold-start handling: when zero candidates score above zero, the prompt writes the cold-start note (`"No prior context retrieved — first REQ in this area."`) in the Retrieved Context section and proceeds without retrieved bodies.
- [ ] The old 3-tier grep logic at Step 1.3 is removed (not left as fallback). Per REQ-258 AC-10.
- [ ] The prompt still passes the preflight check for `.forge/context/project-overview.md`.
- [ ] The modified prompt's prompt.md file is valid markdown and its frontmatter parses.

## Technical Notes

### Current state of spec/prompt.md (reference — from toolkit HEAD)

The current `spec/prompt.md` has four steps (Understand the Request, Determine Next REQ ID, Create the Requirement Spec, Present for Review) plus Prerequisites, Quality Checklist. Step 1 has sub-steps including the lesson grep at 1.3. Step 2 is the atomic REQ ID counter logic (must be preserved). Step 3 is the template-based spec authoring.
...
1. Read the feature request in `$ARGUMENTS`. Extract likely area signals:
   - Which narrow component does this touch? (e.g., `API/auth`, `iOS/SwiftUI`, `Copilot Forge/spec`)
   - Which broad domain? (e.g., `auth`, `payments`, `ui`)
   - Which tech stack layers? (e.g., `express`, `postgres`, `swiftui`)
...
3. **Unified retrieval across corpora** — perform weighted-score retrieval over three corpora using the query from Step 0.5. **Remove** the prior 3-tier grep logic; this step is the only retrieval behavior.
   ...
   a. **Enumerate candidate files** with three Grep passes (paths relative to project root):
      - `.forge/knowledge/lessons/*.md` — no status filter
      - `.forge/specs/*/requirement.md` — include only where frontmatter `status` is `approved`, `in-progress`, or `deployed`
      - `.forge/bugs/*.md` — include only where frontmatter `status` is `resolved`
...
REQ-258's own requirement.md at `.forge/specs/REQ-258-unified-retrieval-spec-pilot/requirement.md` demonstrates the expected output shape (self-tagged frontmatter + `## Retrieved Context` section at the end with cold-start note). Use it as a reference for what a Step 3 output should look like.
