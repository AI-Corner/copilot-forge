...
created: 2026-04-20
updated: 2026-04-20
component: "Copilot Forge/backfill"
domain: "Copilot Forge"
stack: ["markdown", "bash", "copilot-prompts"]
...
|--------|---------------------------------|----|----|----|----|----|
| Lessons | `.forge/knowledge/lessons/*.md` | 136 | 36 | 27 | 13 | see below |
| Specs | `.forge/specs/*/requirement.md` | 160 | 45 | 27 | 13 | YAML |
| Bugs | `.forge/bugs/*.md` | 10 | 1 | 0 | 0 | YAML |
...
| `domain` | string (single) | broad area, e.g. `"auth"`, `"payments"`, `"ui"`, `"Copilot Forge"` |
| `stack` | string[] | tech layers touched, e.g. `["express", "postgres"]` |
...
- [ ] BR-1: **Per-corpus commits.** Each corpus tier in each repo gets its own commit. Commit messages: `chore(Copilot Forge): backfill tags for <corpus> [REQ-262]`. Example: one commit for atelier-fashion specs, one for atelier-fashion bugs, one for atelier-fashion lessons.
...
- [ ] BR-5: **Assumption and decision corpora are explicitly out of scope.** `.forge/knowledge/assumptions/` (6 files in atelier-fashion) and `.forge/knowledge/decisions/` (2 files in atelier-fashion) are NOT backfilled by this REQ because REQ-258's retriever does not read them. A separate REQ handles those corpora when/if they become retrievable.
...
- [ ] AC-2: No legacy lesson is left without a YAML frontmatter block. After backfill, every `.forge/knowledge/lessons/*.md` file starts with `---` on line 1 (or line 1-after-a-preserved-html-comment-header per the lesson template's filename-convention note).
- [ ] AC-3: Per-corpus commit discipline is observed — `git log` in each repo shows discrete commits titled `chore(Copilot Forge): backfill tags for <corpus> [REQ-262]` (one per corpus per repo).
- [ ] AC-4: All 4 repos have one open-and-merged PR titled `chore(Copilot Forge): backfill tag frontmatter [REQ-262]` (or similar) with per-corpus commits visible in the history.
...
- [ ] AC-7: Assumption and decision corpora in atelier-fashion are UNCHANGED by this backfill (BR-5 enforcement). `git diff` should show no modifications to `.forge/knowledge/assumptions/*.md` or `.forge/knowledge/decisions/*.md`.
...
- The `.forge/context/taxonomy.md` files in consumer repos (copied from the canonical stub) contain representative values but may not be exhaustive. Authors extend taxonomies as needed; the backfill agent may propose new values that aren't yet in the taxonomy if justified.
...
- Backfilling `.forge/knowledge/assumptions/` (6 files, atelier-fashion only) and `.forge/knowledge/decisions/` (2 files, atelier-fashion only). Deferred to a follow-up REQ when those corpora become retrievable.
...
cat .forge | head -20
# Expect: line 9 says "You are writing a requirement spec following the spec-driven Copilot Forge process." (no "Atelier Fashion" hardcode)
# Expect: line ~19 says "Taxonomy: !`cat .forge/context/taxonomy.md..."

# 2. All 4 consumer repos have taxonomy.md
for R in atelier-fashion admin-api atelier-web infrastructure; do
  echo "--- $R ---"
  ls -la ~/Documents/GitHub/$R/.forge/context/taxonomy.md 2>/dev/null || echo "MISSING"
done
...
for R in atelier-fashion admin-api atelier-web infrastructure; do
  echo "--- $R ---"
  grep -c "^component:\|^domain:\|^stack:\|^concerns:\|^tags:" ~/Documents/GitHub/$R/.forge/templates/requirement-template.md
done
...
3. Read .forge/context/taxonomy.md for project vocabulary
4. For each corpus in {specs, bugs, lessons}:
   ...
   e. git commit -m "chore(Copilot Forge): backfill tags for <corpus> [REQ-262]"
5. git push -u origin chore/req-262-tag-backfill
6. gh pr create --title "chore(Copilot Forge): backfill tag frontmatter [REQ-262]" --body <see template>
...
- **Across repos**: sequential is safer. Running 4 repos in parallel risks context contamination (agent mixing taxonomies). If time pressure demands it, use 4 separate Copilot Forge sessions, one per repo.

### Commit message template (per corpus per repo)

```
chore(Copilot Forge): backfill tags for <corpus> [REQ-262]

Adds unified retrieval tag frontmatter (component, domain, stack,
concerns, tags) to N files in .forge/<corpus>/.
...
Part of REQ-262. Toolkit REQ: copilot-forge#12.
```
...
Backfill unified retrieval tag frontmatter across all in-scope files in this repo's .forge/ corpus, per [REQ-262](https://github.com/atelier-fashion/copilot-forge/blob/main/.forge/specs/REQ-262-backfill-tag-frontmatter/requirement.md).

## Corpus backfilled
- `.forge/knowledge/lessons/`: N files (tier: Y)
- `.forge/specs/*/requirement.md`: M files (tier: 1)
- `.forge/bugs/*.md`: K files (tier: 1)
- Total: X files
...
🤖 Generated with [Copilot Forge](https://github.com/features/copilot)
```
