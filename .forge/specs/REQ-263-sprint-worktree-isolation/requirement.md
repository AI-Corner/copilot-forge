...
- [ ] BR-7: **Dispatch prompt is the contract.** The exact dispatch-prompt template in `sprint/prompt.md` Step 3 is normative — the orchestrator MUST emit a `WORKTREE PATH (mandatory): <abs-path>` line per dispatched agent. The line format is fixed so the receiving `#proceed` prompt can parse it deterministically.
- [ ] BR-8: **Cross-repo coverage.** For cross-repo REQs, the orchestrator declares the **primary repo's** worktree path in the dispatch prompt. Sibling worktree paths are still resolved by `#proceed` Step 0 from `.forge/config.yml`, but each sibling path goes through the same BR-3 validation gate before `git worktree add`.
...
- Generalizing the "orchestrator declares boundary" pattern to other multi-agent prompts (`#review`, `#architect` parallel dispatch). May be worth a follow-up sweep but is not required for this REQ.
 ...
 **Agent prompt for each REQ**:
 ```
 Run the #proceed prompt for REQ-xxx in the repository at [current repo path].
