...
Update `sprint/prompt.md` so that:
...
- `sprint/prompt.md` — Step 2 gets a new pre-flight check (added to the existing pre-flight table) and a new "Issue" reason. Step 3 dispatch prompt template gets the `WORKTREE PATH (mandatory): <abs-path>` line plus a one-line reminder of the worktree-isolation rules for the receiving agent.

## Acceptance Criteria

- [ ] `sprint/prompt.md` Step 3 dispatch prompt includes the contract line `WORKTREE PATH (mandatory): <absolute-path>` exactly as fixed in `architecture.md`. (Covers BR-1, BR-7.) The orchestrator computes `<absolute-path>` as `<repo-path>/.worktrees/<REQ-id>` for each dispatched agent.
