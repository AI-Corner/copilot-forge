...
2. **Receiving prompt validates the declared path** before `git worktree add`: consult `git worktree list --porcelain` and fail loud if the target path is already registered to a different branch. Never silently reuse an existing worktree.
