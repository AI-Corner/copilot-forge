...
Here is the end-to-end journey of a feature built using Copilot Forge. You can run these steps manually or orchestrate them automatically using the `#forge-proceed` command.

```mermaid
graph TD
    Start([User Request / Idea]) --> Spec[<b>#forge-spec</b><br/>Define Requirements & Retrieve Context]
    
    subgraph Planning Phase
    Spec --> V1[<b>#forge-validate</b><br/>Check Specs]
    V1 --> Architect[<b>#forge-architect</b><br/>Design & Break into Tasks]
    Architect --> V2[<b>#forge-validate</b><br/>Check Architecture]
    end
    
    subgraph Implementation Phase
    V2 --> TDD[<b>#forge-tdd</b><br/>Generate Failing Tests]
    TDD --> Implement[<b>Task Implementation</b><br/>Write Code]
    end
    
    subgraph Review & Deployment Phase
    Implement --> Reflect[<b>#forge-reflect</b><br/>Self-Review]
    Reflect --> Review[<b>#forge-review</b><br/>Multi-Dimensional Review]
    Review --> Canary[<b>#forge-canary</b><br/>Optional Canary Deploy]
    Canary --> PR[Create Pull Request]
    Review --> PR
    PR --> Wrapup[<b>#forge-wrapup</b><br/>Deploy, Document & Clean]
    end
...
### Flow Breakdown:
1. **Planning**: `#forge-spec` gathers requirements, `#forge-architect` breaks them into technical tasks.
   > **The Double Validation Gate:** `#forge-validate` appears twice because two different artifacts get gated: the spec *(does the requirement express what we actually want?)* and the architecture *(do the tasks cover the spec, with correct dependencies and repo routing?)*. Both gates are hard — a failing validation loops back, it does not proceed.
2. **Implementation**: `#forge-tdd` writes tests first (Red-Green-Refactor), followed by actual coding.
3. **Review**: `#forge-reflect` ensures the code matches the tasks, `#forge-review` brings in "specialized agents" to audit code quality, correctness, architecture, tests, and security.
4. **Wrap-up**: After tests pass and PRs merge, `#forge-wrapup` commits changes, generates docs, and cleans up.
...
*   **The Orchestrator (`#forge-proceed`)**: The agent reads the state file, identifies the "Incomplete" tasks, and enters a recursive execution loop. It automatically triggers `#forge-spec`, `#forge-architect`, `#forge-tdd`, etc., without pausing to ask for permission for each transition.
...
*   **Halt Conditions**: The contract only pauses at specific, declared "Hard Halts":
    1. **Validation Failure**: The `#forge-validate` agent returns a `FAIL`.
...
What makes Forge accurate is how it "wires" the AI. When a command like `#forge-proceed` is run, the AI merges three distinct data streams to form its prompt:
...
When `#forge-spec` is invoked, it doesn't just write a requirement from scratch. It acts as a **Retriever**, pulling prior context from the `.forge/knowledge/` corpus. This ensures that mistakes aren't repeated and cross-cutting concerns are baked into the new spec automatically.
...
During the `#forge-review` phase (Phase 5 of `#forge-proceed`), Copilot dispatches specialized "Auditor" and "Reviewer" agents. Crucially, **reviewers act under constraints.** They audit the code against conventions and security rules but do not implement the features. Only the "Implementer" agent is responsible for writing the production code. 
...
* **Orchestrated Merge:** During `#forge-wrapup`, PRs are merged in a strict, predefined `merge_order` to prevent breaking changes.

```mermaid
flowchart TD
    Start[User runs <b>#forge-proceed</b><br/>from primary repo] --> Resolve[Resolve siblings from config.yml]
...
### 6. Multi-REQ Orchestration (`#forge-sprint`)
For bulk work, the `#forge-sprint` command orchestrates multiple `#forge-proceed` pipelines. Copilot Forge manages atomic REQ counters and isolated worktrees for each feature, ensuring that multiple features can be developed concurrently without Git state collisions.
...
* **Core Pipeline**: `#forge-spec`, `#forge-architect`, `#forge-tdd`, `#forge-reflect`, `#forge-review`, `#forge-wrapup`
* **Orchestrators**: `#forge-proceed` (runs a single feature end-to-end), `#forge-sprint` (runs multiple features).
* **Maintenance & Audit**: `#forge-analyze`, `#forge-optimize`, `#forge-security-scan`.
* **Other Utilities**: `#forge-init`, `#forge-bugfix`.

### 2. Agents (The Specialized Personas)
Agents are specialized reference checklists located inside `.github/prompts/agents/`. 
They **do not run on their own**. Instead, they get "hired" by the Skills. For example, when the `#forge-review` Skill runs, it dynamically instructs Copilot to adopt the `security-auditor.md` persona. Breaking them out into a subfolder keeps the master prompts clean and prevents context pollution.
...
**The Relationship:** When you run a **Skill** (`#forge-review`), it adopts an **Agent** persona (`security-auditor`), and if it needs to write a report, it uses a **Template** to format it!
...
* **`context/`:** Keeps living documents of how the system is built (like `architecture.md` and `variables.md`). It is automatically updated via the `#forge-analyze` tool to resolve documentation drift.
* **`specs/`:** Holds the feature specifications (`REQ-*.md`) generated by `#forge-spec`, and the step-by-step coding instructions (`tasks/`) generated by `#forge-architect`.
...
1. Show how `#forge-init` scaffolds the project and `.forge/` directory.
2. Run `#forge-spec` and highlight how it **retrieves prior context** from `.forge/knowledge/` so it doesn't make past mistakes.
3. Show `#forge-architect` breaking requirements into concrete tasks across **multiple repositories** inside `.forge/specs/tasks/`.
