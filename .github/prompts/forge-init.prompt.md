...
**codebase-state.md** — real empirical state, auto-generated by scanning `package.json`, `pom.xml`, etc:
```markdown
---
last_reconciled: {current_date}
---

## Stacks
| Region | Language | Framework | Test Runner | Lint |
|---|---|---|---|---|
| src/ | {Language} | {Framework} | {Test Tool} | {Lint Tool} |
...
**quality-radar.md** — scaffold an empty scorecard (to be filled by `#forge-analyze`):
```markdown
# Quality Radar

*Run `#forge-analyze` to populate this scorecard.*

| Dimension | Score | Summary |
|-----------|-------|---------|
| Code Quality | - | Run #forge-analyze |
| Convention Compliance | - | Run #forge-analyze |
| Security | - | Run #forge-analyze |
| Testing | - | Run #forge-analyze |
| Repo Hygiene | - | Run #forge-analyze |
| **Overall** | **-** | |
```

**code-debt.md** — scaffold an empty ledger (to be filled by `#forge-analyze` and `#forge-review`):
```markdown
# Code Debt Ledger

*Tracked technical debt. Items are appended here by `#forge-analyze` and `#forge-review`.*

| Date | Source | Severity | Description |
|------|--------|----------|-------------|
| | | | |
```
...
Check if `.forge/.env.local` exists. If not, copy `templates/env-local-template.env` to `.forge/.env.local`. Advise the user to open this file and configure their `GITLAB_TOKEN` and IDs if they plan to use `#forge-issue-epic-creation`.

### Step 9: Summary
1. Display the created directory structure
2. Explain the Copilot Forge workflow: `#forge-spec` → `#forge-validate` → `#forge-architect` → `#forge-validate` → `#forge-issue-epic-creation` → implement...
3. Suggest next step: "Run `#forge-spec` to write your first requirement spec."
