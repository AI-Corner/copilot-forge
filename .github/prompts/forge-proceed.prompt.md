...
# proceed — End-to-End Copilot Forge Pipeline
...
3. For each task (in dependency order):
   ...
   - **Refresh context**: run `.\scripts\forge-context.ps1 -ReqId REQ-xxx -TaskId TASK-xxx` (or `.sh` equivalent) to regenerate `.forge/.active-context.md`. Read this file before touching any code.
   ...
   - Run the test suite: `.\scripts\forge-test.ps1 -ReqId REQ-xxx` (or `.sh` equivalent). Read `.forge/.last-test-run.md` to verify previously failing TDD tests now pass (the "Green" phase).
...
- `gateFailures`: increment for each `.\scripts\forge-gate.ps1` (or `.sh`) failure
