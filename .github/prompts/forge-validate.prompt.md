...
Before proceeding, use the codebase tool to verify `.forge/specs/` exists. If it doesn't, stop and tell the user: "The `.forge/` structure hasn't been initialized. Run `#forge-init` first."
...
- [ ] Architecture follows existing patterns from `.forge/context/rules/architecture.rules.md`
- [ ] New ADRs include rationale (not just decisions)
- [ ] Data model changes are compatible with existing schema
- [ ] API endpoint design follows REST conventions from `.forge/context/rules/conventions.rules.md`
...
- [ ] Code follows conventions from `.forge/context/rules/conventions.rules.md`
...
- Spec validated ? "Ready for `#forge-architect`"
- Architecture validated ? "Ready for implementation"
- Tasks validated ? "Ready for implementation"
- Implementation validated ? "Ready for `#forge-review`"

## Internal Reference
- **Incoming Skill Dependencies**: `#forge-proceed`, `#forge-spec`
