...
Use the codebase tool to verify `.forge/bugs/` exists. If it doesn't, stop: "The `.forge/` structure hasn't been initialized. Run `#forge-init` first."

Read `.forge/config.yml` and `.forge/context/rules/conventions.rules.md` via the codebase tool.
...
3. Implement the fix following project conventions from `.forge/context/rules/conventions.rules.md`.
...
Invoke `#forge-canary` for each affected service. If all canaries pass, continue. If any fails, halt and surface the failure.
...
- **Outgoing Skill Dependencies**: `#forge-canary`, `#forge-wrapup`
