...
```powershell
.\scripts\forge-gate.ps1 -Phase tdd # (Use ./scripts/forge-gate.sh on Linux/Mac)
```
...
Do NOT ask the user to run tests manually. Run `.\scripts\forge-test.ps1` (or `.sh`) via terminal instead:

```powershell
.\scripts\forge-test.ps1 -ReqId REQ-xxx # (Use ./scripts/forge-test.sh on Linux/Mac)
```
...
3. Re-run `.\scripts\forge-test.ps1` (or `.sh`) and re-read `.forge/.last-test-run.md`.
