...
3. Suggest which items could become Copilot Forge requirements (candidates for `#forge-spec`).
...
2. **Update `code-debt.md`**: Append all items from the "Technical Debt" and "Improvement Opportunities" sections to `.forge/context/code-debt.md` in the ledger table format `| Date | Source | Severity | Description |` (Source = `#forge-analyze`).
