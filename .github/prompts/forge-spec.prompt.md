...
Before proceeding:
1. Use the codebase tool to verify `.forge/context/project-overview.md` exists. If it doesn't, stop and tell the user: "The `.forge/` structure hasn't been initialized. Run `#forge-init` first."
...
2. Decision:
   ...
   - If ALL are NO â†’ The requirement is suitable for VIBE. Stop immediately and ask the user: *"This looks like a small, isolated change suitable for Vibe Coding. Would you like to use the lightweight `#forge-vibe` workflow instead? If yes, please run `#forge-vibe [your requirement]`."* Do not proceed further until the user decides.
...
- **Outgoing Skill Dependencies**: `#forge-validate`
