...
2. **Pre-Commit Security Scan**: Execute the logic from `#forge-security-scan`. Read the uncommitted diff and scan for hardcoded secrets, passwords, or tokens.
...
#### Learning Candidates (Inbox)
Review the implementation process for new knowledge:
- **Architectural Decisions**: Did an emergent global shift occur that sets a new reusable standard?
...
- If deferred items exist: "Consider creating `#forge-spec` for deferred items: [list]"
...
## Internal Reference
- **Incoming Skill Dependencies**: `#forge-proceed`, `#forge-review`, `#forge-bugfix`
