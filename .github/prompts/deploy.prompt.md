...
Before proceeding:
1. Use the codebase tool to verify `.forge/context/deployment.md` exists. If it doesn't, stop and tell the user: "The deployment context hasn't been initialized. Ensure `.forge/context/deployment.md` exists or run `#init` again."
2. Read `.forge/context/deployment.md` for deployment workflows.

## Instructions

### Step 1: Determine Deployment Target
1. If the user did not specify a target (e.g., they just typed `#deploy`), assume "local" but ask for confirmation: "Deploying locally. If you meant to deploy to staging or production, please specify: `#deploy staging`".
